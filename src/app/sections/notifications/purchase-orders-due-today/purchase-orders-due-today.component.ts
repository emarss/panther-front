...
@Component({
  selector: 'app-purchase-orders-due-today',
  templateUrl: './purchase-orders-due-today.component.html',
  styleUrls: ['./purchase-orders-due-today.component.scss'],
})
export class PurchaseOrdersDueTodayComponent {
  public loading = true;
  ...
  private getPurchaseOrdersList() {
    ...
    this.purchaseOrderService
      .dueToday(this.paginationOptions)
  ...
  exportData(type: string) {
    this.loading = true;
    this.purchaseOrderService
      .exportDueToday(type, this.searchKey, this.paginationOptions)
      .subscribe({
        next: (data) => {
          saveAs(data as Blob, `Purchase Orders Due ${moment().format('DD-MM-YYYY')} Exported - ${moment()}.${type}`);
          this.loading = false;
        },
        error: (ex) => {
          this.loading = false;
