...
export class ReceiptsComponent {
  ...
  accountReceiptsList!: Array<Receipt>;
  public sub: any;
  ...
  ngOnInit() {
    this.getAccountReceipts();
  }

  getAccountReceipts() {
    this.loading = true;
    this.accountService
      .getAccountReceipts(this.account.uuid, this.paginationOptions)
      .subscribe((res: PaginatedResults) => {
        this.paginatedResults = res;
        this.accountReceiptsList = res.data.map((u) => new Receipt(u));
        this.loading = false;
      });
  }
  ...
  confirmDeleteReceipt(receipt: Receipt) {
    this.confirmDialogService.showConfirmDialog({
      ...
      message: `Are you sure you want to delete receipt #${receipt.number}`,
      actionText: 'Delete',
      ...
      action: () => {
        ...
        this.receiptService.delete(receipt.uuid).subscribe({
          next: (res: Receipt) => {
            this.accountReceiptsList = this.accountReceiptsList.filter(
              (u) => u.uuid != receipt.uuid
            );
            this.toastService.showSuccess(
              `Receipt receipt #${receipt.number} has been deleted successfully.`
            );
            this.loading = false;
          },
          error: (err: any) => {
